...
model.eval()
try:
    torch.onnx.export(model, dummy_input, "nn.onnx", input_names=['features'], output_names=['price_log'], opset_version=11)
except Exception as e:
    traced_model = torch.jit.trace(model, dummy_input)
    traced_model.save("nn.onnx")
with open("README.txt", "w", encoding="utf-8") as f:
    f.write(f"Stacking Formula:\nfinal_price = {meta.intercept_:.4f} + {meta.coef_[0]:.4f} * xgb_pred + {meta.coef_[1]:.4f} * nn_pred\n")
print("Saved: xgb_model.json, nn.onnx, README.txt")
